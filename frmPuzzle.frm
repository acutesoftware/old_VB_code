...
Public Function RandomShuffle(times As Integer)
    'this function does 40 random shuffles, then moves ALL the
    'squares at least once and then 50 random shifts of the 'blank' sqaure
    Dim num As Integer
    Dim rndNum As Integer   'shuffle direction: 1=left,2=r, 3=up,3=down
    Dim x1 As Long, x2 As Long, y1 As Long, y2 As Long
       ' MsgBox "Random Shuffling"
        
    For num = 1 To times
        rndNum = Rnd(1) * 3 + 1
        Debug.Print "Random Number " & Str$(num) & " = " & Str$(rndNum)
        Select Case rndNum
        Case 1
            If blankx > 1 Then
                SwapCells blankx, blanky, blankx - 1, blanky
                blankx = blankx - 1
                'MsgBox "moved left"
            End If
        Case 2
        ...
        Case 3
            If blanky > 1 Then
                SwapCells blankx, blanky, blankx, blanky - 1
                blanky = blanky - 1
                'MsgBox "Moved up"
            End If
        Case 4
            If blanky < 4 Then
                frmPuzzle.SwapCells blankx, blanky, blankx, blanky + 1
                blanky = blanky + 1
                'MsgBox "Moved down"
            End If
        End Select
...
'        lblInfo.Caption = "MOVING ACROSS from grid" & Str$(x_orig) & Str$(y) & "  to grid" & Str$(x_dest) & Str$(y)
        ...
        End Select
End Sub
Public Sub SwapCells(x1 As Long, y1 As Long, x2 As Long, y2 As Long)
...
'    End If
  ...
  'WORKS
  If Text1.Visible = True Then  'this means we are still debugging
    DisplayGrid
...
Private Sub cmdShuffle_Click()
    RandomShuffle 10
    For j = 1 To 10  '5 big shuffles
      For i = 1 To 4 'make sure its on the left (its biased that way)
        If blankx > 1 Then SwapCells blankx - 1, blanky, blankx, blanky: blankx = blankx - 1
      Next i
      RandomShuffle 20
    Next j
